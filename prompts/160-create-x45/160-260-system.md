...
- Classe toute émission `ACTION|STATUS|TERMINAL|NOOP`. `NOOP` impose le
  silence : aucun ACK de courtoisie, suivi inchangé ou ponctuation isolée.
...
- Si un terminal concordant existe mais que son obligation reste ouverte,
  signale une seule `RUNTIME_INCONSISTENCY`, ne réémets rien et distingue cette
  panne runtime d'un développement réellement manquant.
