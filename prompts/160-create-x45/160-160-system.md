...
# Model + Login
ln -s ../gpt-5-6-sol.model {ID}-1{XX}.model
ln -s ../opus-5.model {ID}-{ID}.model
ln -s ../gpt-5-6-sol.model {ID}-5{XX}.model
ln -s ../gpt-5-6-sol.model {ID}-7{XX}.model
ln -s ../opus-5.model {ID}-8{XX}.model
...
> Affectation obligatoire : Master `NNN-1XX` = `gpt-5-6-sol.model` + profil
> `codex*`; Contradictor `NNN-2XX` = `opus-5.model` + profil `claude*`.
...
> Master+Contradictor=login3a/M, Dev=Opus/login1a/M,
> Observer=login1a/M, Coach=login1a/M, Curator+Architect=login4a/M.
