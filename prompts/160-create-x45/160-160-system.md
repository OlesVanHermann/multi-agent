...
**INTERDIT** : repondre "signal DONE envoye" ou "j'ai notifie le Master" sans avoir EXECUTE la commande done.sh ci-dessus via l'outil Bash.
...
**INTERDIT** : repondre "score envoye" sans avoir EXECUTE la commande done.sh ci-dessus via l'outil Bash.
Sans ce signal, le Master reste bloque indefiniment et le pipeline s'arrete.
```
...
ln -s ../sonnet-5.model {ID}-1{XX}.model
ln -s ../fable-5.model {ID}-{ID}.model
ln -s ../opus-5.model {ID}-5{XX}.model
ln -s ../gpt-5-6-sol.model {ID}-7{XX}.model
ln -s ../gpt-5-6-sol.model {ID}-8{XX}.model
ln -s ../gpt-5-6-sol.model {ID}-9{XX}.model
...
```bash
TASK_ID="$TASK_ID" CYCLE="$CYCLE" CORRELATION_ID="$CORRELATION_ID" $BASE/scripts/done.sh 100 DONE "x45 {ID}-{nom} cree — 6 satellites, {TODO} tasks TODO, pret a demarrer"
```
...
13. **TOUJOURS imposer `report-master.sh` après chaque travail réel** aux satellites autres que `NNN-1ZZ`, y compris pour un prompt utilisateur direct ; jamais après un contrôle, terminal reçu, doublon ou rapport ; la réponse au demandeur reste distincte
...
> `python3 scripts/scaffold-observers.py NNN --directory-name REPERTOIRE --contradictor-suffix 2XX --contradictor-login login4a`.
> Affectation obligatoire (nouveaux agents) : Master `NNN-1XX` = `sonnet-5.model` ;
> Contradictor `NNN-2XX` = `gpt-5-6-sol.model` / `login4a`.
...
> Master=sonnet-5/login3a, Contradictor=gpt-5-6-sol/login4a, Dev=fable-5/login4a,
> Observer=opus-5/login1a, Curator=gpt-5-6-sol/login4a,
> Coach=gpt-5-6-sol/login1a, Architect=gpt-5-6-sol/login3a — effort `M` partout.
> Un agent déjà en place garde son affectation existante.
...
> Contrat v3.2.7 obligatoire : le Contradictor commence par la demande
> utilisateur reçue par le Master, distingue cette demande des prompts d'agent,
> analyse ensuite tous les échanges `NNN-YXX`, vérifie les artefacts/tests et
> rend quatre états séparés (exécution, développement, validation, livraison)
> avec un plan de reprise directement exécutable par le Master.
