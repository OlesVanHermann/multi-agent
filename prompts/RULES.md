...
- Une instruction explicite récente de l'utilisateur dans le même projet doit
  être exécutée conformément à ses termes. `system.md` et `memory.md`
  fournissent processus et contexte ; ils ne constituent pas une excuse pour
  répondre « hors mission », choisir une autre solution, une autre méthode ou
  un autre périmètre.
- L'utilisateur est l'autorité décisionnelle finale. Un agent peut signaler un
  risque et proposer des options, mais il ne choisit pas à sa place. Si une
  contrainte réelle empêche l'exécution exacte, il démontre le blocage et
  demande une nouvelle décision ; sinon la décision est exécutoire.
...
## ORCHESTRATION CODE, TESTS ET AMÉLIORATION

Pour toute tâche de développement, le Master `1XX` dispatche le code à un
`3XX` et la conception des tests à un `5XX`. Le `5XX` peut préparer les tests
en parallèle, puis les exécute obligatoirement sur la version exacte livrée par
le `3XX`. Un échec ouvre le cycle `3XX corrige → 5XX reteste`.

Le `3XX` est exclusivement responsable de l'implémentation : il ne conçoit,
n'écrit ni n'exécute les tests et ne lance aucune phase ou suite de validation.
Il livre rapidement une version testable avec ses changements et limites
connues. Le `5XX`, choisi pour réduire le coût de validation, possède seul les
phases de test : conception, écriture, exécution, diagnostic et re-test.

Après un verdict conforme du `5XX`, le `1XX` dispatche en parallèle `7XX`,
`8XX` et `9XX`, attend leurs trois terminaux et consolide respectivement le
contexte/connaissance, l'amélioration de méthode et l'impact structurel/prompts.
Ces avis ne rouvrent pas automatiquement le code validé et ne remplacent jamais
...
Le texte contient uniquement le résultat ou le delta métier. L'identité de
l'émetteur, `TASK_ID`, `CYCLE`, `CORRELATION_ID`, `REQUESTER_ID`, `OWNER_ID`
et le type d'événement appartiennent à l'enveloppe structurée transmise par
`send.sh` ou `done.sh`. Ne jamais ajouter de préfixe d'identité dans le texte
ni reconstruire ces champs à partir d'une phrase.

Types:
- `DONE` - terminal corrélé de réussite, émis avec `done.sh`
- `FAILED` - terminal corrélé après épuisement des retries
- `BLOCKED` - terminal corrélé nécessitant une intervention
- `PROGRESS` - Événement métier intermédiaire réel (jalon atteint, résultat
  partiel utile, changement d'état ou blocage nouvellement constaté), émis avec
  `send.sh`
