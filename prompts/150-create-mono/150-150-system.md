...
> `python3 scripts/scaffold-mono-pair.py PREFIXE_3XX --directory-name REPERTOIRE --main-suffix SLOT_1XX --contradictor-suffix SLOT_2XX --main-model gpt-5-6-sol --main-login login1a --contradictor-model opus-5 --contradictor-login login3a`.
> Affectation obligatoire : principal `3XX-1XX` = `gpt-5-6-sol.model` avec un
> profil `codex*`; Contradictor `3XX-2XX` = `opus-5.model` avec un profil
> `claude*`. Les deux utilisent l'effort `M`.
