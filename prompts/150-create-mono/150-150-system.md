...
LOGIN="{login}"   # ex: default, login1a, login1b
...
```bash
TASK_ID="$TASK_ID" CYCLE="$CYCLE" CORRELATION_ID="$CORRELATION_ID" bash $BASE/scripts/done.sh 100 DONE "{description}"
```
...
```bash
TASK_ID="$TASK_ID" CYCLE="$CYCLE" CORRELATION_ID="$CORRELATION_ID" $BASE/scripts/done.sh 100 DONE "mono ${ID}-${NOM} créé dans prompts/ — prêt à démarrer"
```
...
9. **TOUJOURS** imposer `report-master.sh` après chaque travail réel aux agents de triangle autres que leur `NNN-1ZZ`, même après un prompt utilisateur direct ; jamais en réponse à un contrôle, terminal reçu, doublon ou rapport
...
> Le terminal au demandeur et le `MASTER_REPORT` sont deux obligations distinctes lorsque leurs cibles diffèrent.
...
> `python3 scripts/scaffold-mono-pair.py PREFIXE_3XX --directory-name REPERTOIRE --main-suffix SLOT_1XX --contradictor-suffix SLOT_2XX --main-model opus-5 --main-login login3a --contradictor-model gpt-5-6-sol --contradictor-login login4a`.
> Affectation obligatoire (nouveaux agents) : principal `3XX-1XX` = `opus-5.model`
> avec `login3a`; Contradictor `3XX-2XX` = `gpt-5-6-sol.model` avec
> `login4a`. Les deux utilisent l'effort `M`. Un agent déjà en place
> garde son affectation existante.
...
> utilisateur reçue par le principal et ses amendements, la distingue des
> fichiers `system.md`/`memory.md`/`methodology.md`, puis confronte les échanges
> de la paire aux preuves physiques. Sa conclusion qualifie séparément
> exécution, développement, validation et livraison, et fournit le plan de
> correction jusqu'au résultat attendu.
