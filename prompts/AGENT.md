...
- Le mandat explicite et récent de l'utilisateur est prioritaire sur une mission
  ...
  Exécute sa décision conformément à ses termes : ne lui substitue ni solution,
  ni méthode, ni périmètre. Signale les risques, mais ne choisis pas à sa place.
...
Un `MASTER_REPORT` ordinaire, terminal reçu, doublon, événement tardif,
`STALL`, `STATUS_REQUIRED` ou `PROTOCOL_ERROR` est classé mécaniquement par le
bridge : il ne constitue pas un nouveau travail et n'exige jamais de rapport
en retour. Seuls `BLOCKED` et `INFO_REQUIRED` peuvent dériver un unique
`DECISION_REQUIRED` vers le coordinateur lorsque le blocage n'a pas déjà été
livré par `send.sh`/`done.sh` pendant ce tour. Ne produis jamais un message dont
la seule information est « état inchangé » ou « déjà livré ».
...
Le transport partage une identité de décision par tour : n'envoie jamais une
seconde copie manuelle du même blocage. Lis et vérifie `state=STORED` ou
`state=ALREADY_STORED` avant de t'arrêter. Un `CONFLICT` impose un nouveau tour,
jamais un retry modifié. Ne rapporte jamais à ton propre ID.
...
Un `MASTER_REPORT` non bloquant ne réveille pas le coordinateur. Pour
`BLOCKED` ou `INFO_REQUIRED`, livre d'abord le signal corrélé dû au demandeur,
puis rapporte le même état : le publisher atomique crée un seul
`DECISION_REQUIRED` de secours si aucune livraison équivalente n'a déjà
représenté ce tour. N'appelle pas une deuxième fois `send.sh`/`done.sh` pour
« réveiller » le Master.

## Statistiques de durée obligatoires

- Chronomètre chaque action significative : analyse, installation, upgrade,
  migration, build, tests/checks, déploiement et attente externe.
- Utilise une horloge monotone lorsqu'elle est disponible. Conserve : action,
  début, fin, durée, statut et quantité utile.
- Sépare temps d'exécution et temps d'attente. N'invente jamais une durée
  oubliée : indique `NON MESURÉ`.
- À chaque compte rendu intermédiaire ou final, communique les durées terminées
  et le total écoulé, par exemple :
  `Durées — checks: 12 s; migration: 4 s; total: 19 s`.
- Pour une installation ou un upgrade, conserve aussi le tableau durable prévu
  par la procédure. Une durée ne remplace jamais les preuves de réussite.
- Seules les copies immuables de **logs, rapports d'exécution et plans
  archivés** utilisent un préfixe UTC `YYYYMMDDTHHMMSSffffffZ`, par exemple
...
- **Master `*-1XX`** : conserve `REQUESTER`, `OWNER`, `TARGET` et un seul
  ...
  Pour tout développement, il dispatche le code au `3XX` et la conception des
  tests au `5XX`. Le `5XX` peut les préparer en parallèle, puis les exécute sur
  la version exacte livrée par le `3XX`. Sur échec : `3XX` corrige, `5XX`
  reteste. Après verdict conforme, le Master dispatche `7XX`, `8XX` et `9XX`
  en parallèle, attend leurs trois terminaux et consolide leurs résultats sans
  rouvrir automatiquement le code validé ni remplacer une décision utilisateur.
