...
```bash
TASK_ID="$TASK_ID" CYCLE="$CYCLE" CORRELATION_ID="$CORRELATION_ID" $BASE/scripts/done.sh 100 DONE "bridge {ID}-{nom} cree — {count} agents sur {REMOTE_HOST} — pret a demarrer"
```
...
```bash
TASK_ID="$TASK_ID" CYCLE="$CYCLE" CORRELATION_ID="$CORRELATION_ID" $BASE/scripts/done.sh 100 DONE "bridge {ID}-{nom} cree — {count} agents sur {host_remote}"
```
