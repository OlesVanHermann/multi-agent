...
> **PROTOCOLE CORRÉLÉ V3.1.10 — OBLIGATOIRE DANS CHAQUE SATELLITE GÉNÉRÉ** :
> conserver `FROM/TASK/CYCLE/CORR`; exactement un événement terminal ;
> enveloppe canonique `FROM|EVENT|TASK|CYCLE|CORR|ARTIFACT|SHA256|DETAIL` ;
> variables de corrélation transmises à `send.sh`/`done.sh`; artefact vérifié
> et hashé; `PROTOCOL_ERROR` si l'enveloppe manque; aucun auto-dispatch,
> polling, timeout de complétion ou chemin d'artefact inventé.
...
- Contrat événementiel : rendre la main après dispatch et reprendre sur événement du bridge
...
```bash
TASK_ID="$TASK_ID" CYCLE="$CYCLE" CORRELATION_ID="$CORRELATION_ID" $BASE/scripts/done.sh 100 DONE "z21 {ID}-{nom} cree — {N} contextes, 6 satellites, pret a demarrer"
```
...
- Après chaque dispatch unique via `send.sh`, rendre immédiatement la main.
- Reprendre le cycle uniquement à la réception de DONE/SCORE/BLOCKED.
- Aucun sleep, polling Redis/tmux, contrôle de session, timeout de complétion,
  retry préventif ou redispatch.
- En cas de blocage reçu, le signaler à l'Architecte ; ne jamais arrêter ou
...
## Reprise événementielle
Après dispatch, terminer immédiatement le tour. Aucun sleep, polling Redis/tmux,
contrôle de session, timeout de complétion ou redispatch. Traiter uniquement
l'événement reçu du bridge, puis rendre de nouveau la main.
```
...
```bash
$BASE/scripts/done.sh {ID}-1{XX} DONE "context=<CTX> | <resume>"
```
...
14. **TOUJOURS imposer send.sh/done.sh** dans Reviewer et Tester ; aucun XADD ou préfixe Redis codé en dur
...
21. **TOUJOURS ajouter le CONTRAT ÉVÉNEMENTIEL** en tête de CHAQUE system.md créé
...
24. **TOUJOURS inclure le contrat événementiel** dans la methodology du Master — rendre la main après dispatch, sans polling, timeout ni redispatch
25. **TOUJOURS imposer `report-master.sh` après chaque travail réel** aux satellites autres que `NNN-1ZZ`, y compris pour un prompt utilisateur direct ; jamais après un contrôle, terminal reçu, doublon ou rapport ; la réponse au demandeur reste distincte
...
> routage, les contextes, enveloppes et scripts sont des moyens appliqués
...
> Création obligatoire : chaque nouveau z21 reçoit un Contradictor local `NNN-2XX` via
...
> L'Contradictor observe le triangle et ses contextes z21, jamais un autre pipeline.
...
> Contrats de rôles v3.2 obligatoires : Curator produit un contexte z21 borné
> et vérifiable; Dev est soumis aux hard gates; Observer sépare hard gates/score
...
> utilisateur reçue par le Master et ses amendements, sans la confondre avec
> les prompts d'agent ou les contextes z21. Il analyse ensuite tous les
> échanges `NNN-YXX`, vérifie les artefacts/tests et fournit au Master le
> verdict d'exécution, développement, validation et livraison ainsi que le
> plan ordonné jusqu'au résultat attendu.
