...
`$BASE/scripts/report-master.sh` après chaque travail réel, même après un
prompt utilisateur direct. Un contrôle, terminal reçu, doublon ou rapport
n'exige aucune réponse. Une livraison corrélée reste distincte.
...
```bash
bash $BASE/scripts/done.sh {ID}-1{XX} DONE "context=<CTX> | <resume>"
```
Le Master ne sait pas que tu as fini tant que tu n'envoies pas ce message.
**INTERDIT** : repondre "signal DONE envoye" ou "j'ai notifie le Master" sans avoir EXECUTE la commande done.sh ci-dessus via l'outil Bash. Sans ce signal, le pipeline s'arrete.
...
```bash
$BASE/scripts/done.sh {ID}-1{XX} ARTIFACT_READY "TEST_RESULT=<PASS|FAIL> context=<CONTEXT> | passed=<n> total=<total> | <resume 1 ligne> | tests=<chemin_fichier_test>"
```
...
```bash
$BASE/scripts/done.sh {ID}-1{XX} SCORE <n> "VERDICT=<BLOCK_DEV|READY_FOR_INTEGRATION|BLOCK_INTEGRATION|ACCEPT_WITH_IMPROVEMENTS> context=<CONTEXT> | hard_gates=<PASS|FAIL> | blocants: N, non-blocants: M | <resume 1 ligne>"
```

### 2. Dev {ID}-{ID} — TOUJOURS (feedback, meme si PASS)
```bash
MESSAGE_EVENT=DECISION_REQUIRED $BASE/scripts/send.sh {ID}-{ID} "REVIEW_RESULT=<PASS|FAIL> context=<CONTEXT> | QUALITY=<n>/100 | <si blocants CODE: fichier:ligne> | <si blocants TEST: fichier attendu> | <non-blocants: liste>"
```
...
```bash
$BASE/scripts/done.sh {ID}-1{XX} BLOCKED "context=<CONTEXT> | <description du blocage>"
```
```
...
```bash
$BASE/scripts/done.sh {ID}-1{XX} DONE "COACH_UPDATE context=<CONTEXT> | memory.md mis a jour, methodology.md <modifie|inchange> | QUALITY=<N>/100 | VERDICT=<APPROUVE|REJETE> | [ALERT: ...si contradiction] [SUGGEST: ...si pattern cross-contextes]"
```

**JAMAIS terminer sans envoyer ce message.**
**INTERDIT** : repondre "signal DONE envoye" sans avoir EXECUTE la commande done.sh ci-dessus via l'outil Bash.
...
```bash
$BASE/scripts/done.sh {ID}-1{XX} DONE "ROLE={ID}-9{XX} | action=<creation|update|audit> context=<CONTEXT> | <resume 1 ligne>"
```
