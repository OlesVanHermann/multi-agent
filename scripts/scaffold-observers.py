...
def scaffold(base, triangle, directory_name, contradictor_login,
    ...
    assignments = {
        agent_id: ("gpt-5-6-sol.model", f"{contradictor_login}.login", "M"),
    }
    ...
    periodic = crontab / f"{agent_id}_360.prompt"
    suspended = periodic.with_suffix(".prompt.suspended")
    if not periodic.exists() and not suspended.exists():
        crontab.mkdir(parents=True, exist_ok=True)
        suspended.write_text(
            f"Collecte une fois l'état réel du triangle {triangle}. Si {target_id} "
