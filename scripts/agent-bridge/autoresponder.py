...
class AutoResponseDecision:
    ...
    A10 (SPEC-OPERATEUR-tui-dialogs-autoresponse) : une décision peut exiger
    une confirmation en deux temps — ``keys`` déplace la sélection, puis
    l'appelant DOIT revérifier que la ligne sélectionnée contient
    ``confirm_selected`` avant d'émettre ``confirm_key``. Sans cette preuve,
    il émet Escape et alerte : jamais de validation à l'aveugle quand
    l'option surlignée par défaut est dangereuse (ex. bascule de modèle).
    """
...
def _option_number(lines, label):
    """Numéro de l'option numérotée dont le libellé contient ``label``.

    A10 : la sélection se fait par le libellé relevé, jamais par une position
    codée en dur — rien ne garantit qu'une option garde son rang d'une
    version de CLI à l'autre. Ambiguïté (0 ou 2+ correspondances) → None.
    """
    matches = []
    for line in lines:
        m = _NUMBERED_OPTION_RE.match(line)
        if m and label in line:
            matches.append(str(m.group("number")))
    return matches[0] if len(matches) == 1 else None


def selected_line_contains(pane_text, config, label, tail_lines=None):
    """La ligne actuellement sélectionnée (préfixe ›/❯) contient-elle label ?

    Utilisé par l'appelant pour la preuve de sélection AVANT la touche de
    confirmation (A10). Sans préfixe de sélection visible : False.
    """
    lines = _clean_lines(
        pane_text, tail_lines or (config or {}).get("tail_lines", 30))
    prefixes = tuple(
        str(item) for item in (config or {}).get("selected_prefixes", []))
    if not prefixes:
        return False
    for line in lines:
        stripped = line.lstrip()
        if stripped.startswith(prefixes) and label in line:
            return True
    return False


def _classify_declared_dialogs(lines, active, config):
    """Dialogues déclaratifs (auto_response.dialogs des markers) — A10.

    Chaque entrée est un relevé de TUI réel : ``require`` (conjonction
    stricte de chaînes, invite de confirmation comprise — un marqueur isolé
    du scrollback ne suffit jamais), ``select_option`` (libellé de l'option
    à sélectionner, numéro résolu dynamiquement), ``keys`` statiques
    éventuelles, puis confirmation en deux temps via
    ``confirm_selected``/``confirm_key``.
    """
    for spec in config.get("dialogs", []) or []:
        kind = str(spec.get("kind", "") or "")
        require = [str(item) for item in (spec.get("require") or [])]
        if not kind or not require:
            continue
        if not all(item in active for item in require):
            continue
        fingerprint = _fingerprint(kind, *require)
        keys = tuple(str(key) for key in (spec.get("keys") or ()))
        select_option = str(spec.get("select_option", "") or "")
        if select_option:
            number = _option_number(lines, select_option)
            if number is None:
                # Layout inattendu : l'option décidée par l'opérateur est
                # introuvable ou ambiguë. Repli sûr : fermer sans choisir.
                return AutoResponseDecision(
                    f"{kind}_escape", ("Escape",), fingerprint)
            keys = (number,) + keys
        if not keys and not spec.get("confirm_key"):
            continue
        return AutoResponseDecision(
            kind, keys, fingerprint,
            confirm_key=str(spec.get("confirm_key", "") or ""),
            confirm_selected=str(spec.get("confirm_selected", "") or ""))
    return None
...
def classify_active_dialog(pane_text, pane_command, markers):
    ...
    # A10 : les dialogues déclarés (relevés, conjonction stricte incluant
    # l'invite) passent AVANT les règles génériques — un écran de bascule de
    # modèle ne doit jamais être gobé par une approval large.
    declared = _classify_declared_dialogs(lines, active, config)
