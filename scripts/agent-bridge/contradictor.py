...
def revise_prompt(triangle):
    """Crée ou remplace le texte du prompt périodique du Master local.

    Un seul candidat existant : remplace son texte, conserve nom/période/statut.
    Aucun candidat : crée `{target}_120.prompt.suspended` (2h, suspendu par
    défaut — l'opérateur décide seul de l'activation). Plusieurs candidats :
    ambigu, refuse plutôt que de deviner lequel toucher.
    """
    directory = prompt_directory(triangle)
    target = role_id(directory, triangle, "1")
    crontab = BASE / "crontab"
    candidates = sorted(
        path for path in crontab.glob(f"{target}_*.prompt*")
        if re.fullmatch(rf"{re.escape(target)}_\d+\.prompt(?:\.suspended)?", path.name)
    )
    if len(candidates) > 1:
        raise SystemExit(
            f"prompt planifié Master ambigu pour {target}: {candidates}"
        )
    replacement = sys.stdin.read().strip()
    if not replacement:
        raise SystemExit("nouveau prompt vide")
    if len(replacement) > MAX_CRONTAB_PROMPT:
        raise SystemExit(f"nouveau prompt trop long (max {MAX_CRONTAB_PROMPT})")
    replacement += "\n"

    if not candidates:
        crontab.mkdir(parents=True, exist_ok=True)
        new_file = crontab / f"{target}_120.prompt.suspended"
        new_file.write_text(replacement)
        print(json.dumps({
            "status": "created", "target": target,
            "file": str(new_file.relative_to(BASE)),
            "period": 120, "suspended": True,
        }, ensure_ascii=False))
        return

    current = candidates[0]
    previous = current.read_text(errors="replace")
    if previous == replacement:
        print("NOOP")
        return
    backup = BASE / "removed" / "contradictor-crontab" / timestamp()
    backup.mkdir(parents=True, exist_ok=False)
    shutil.copy2(current, backup / current.name)
    current.write_text(replacement)
    print(json.dumps({
        "status": "updated", "target": target, "file": str(current.relative_to(BASE)),
        "backup": str((backup / current.name).relative_to(BASE)),
    }, ensure_ascii=False))


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("action", choices=("collect", "send", "revise-prompt"))
    parser.add_argument("triangle", type=validate_triangle)
    args = parser.parse_args()
    actions = {"collect": collect, "send": send, "revise-prompt": revise_prompt}
    actions[args.action](args.triangle)
