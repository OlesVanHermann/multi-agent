...
def scaffold(base, prefix, main_suffix, contradictor_suffix, directory_name,
             ...
             main_model="opus-5", main_login="login3a"):
    ...
    # Affectation canonique des nouveaux mono: principal Opus/login3a et
    # Contradictor Sol/login4a. Les agents existants ne sont pas réaffectés.
    assignments = (
    ...
    periodic = crontab / f"{contradictor_id}_360.prompt"
    suspended = crontab / f"{contradictor_id}_360.prompt.suspended"
    if not periodic.exists() and not suspended.exists():
        crontab.mkdir(parents=True, exist_ok=True)
        suspended.write_text(
            f"Collecte une fois l'état réel du triangle {prefix}. Si {main_id} "
            "n'a aucun prompt planifié, crée son pilotage 120 min désactivé via "
            "revise-prompt ; sinon améliore-le via revise-prompt. Ne change ni "
            "objectifs, nom, période ou statut. NOOP et silence total si aucun "
            "gain concret. L'activation appartient à l'opérateur.\n"
        )
...
def main():
    ...
    parser.add_argument("--contradictor-login", default="login4a")
    parser.add_argument("--main-model", default="opus-5")
    parser.add_argument("--main-login", default="login3a")
