...
ne constitue pas un envoi. Pour `BLOCKED`/`INFO_REQUIRED`, le rapport peut
dériver un unique `DECISION_REQUIRED` de secours ; la même identité de décision
empêche un second réveil si `send.sh`/`done.sh` a déjà livré ce tour. N'envoie
jamais une copie manuelle au Master. Vérifie `state=STORED` ou
`state=ALREADY_STORED` avant de t'arrêter ; `CONFLICT` exige un nouveau tour.
