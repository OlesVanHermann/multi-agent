...
Générer et injecter les secrets automatiquement :

```bash
cd ~/multi-agent
KEYCLOAK_PW=$(bash setup/genpass.sh 2>/dev/null | tail -1)
HEALTH_TK=$(bash setup/genpass.sh 2>/dev/null | tail -1)

sed -i "s/^KEYCLOAK_ADMIN_PASSWORD=.*/KEYCLOAK_ADMIN_PASSWORD=$KEYCLOAK_PW/" setup/secrets.cfg
sed -i "s/^HEALTH_TOKEN=.*/HEALTH_TOKEN=$HEALTH_TK/" setup/secrets.cfg

echo "Secrets générés dans setup/secrets.cfg"
cat setup/secrets.cfg
```

Note : `REDIS_PASSWORD` reste vide — Redis écoute sur localhost uniquement, pas besoin d'auth.
