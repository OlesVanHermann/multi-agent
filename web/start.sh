...
if [ "$MODE" = "dev" ]; then
    ...
    python3 -m uvicorn multi_agent.backend:app --host 127.0.0.1 --port 8050 --ws-ping-interval 25 --ws-ping-timeout 90 --reload &
    BACKEND_PID=$!
    ...
    echo "[backend] Starting FastAPI on :8050..."
    cd "$SCRIPT_DIR/backend"
    python3 -m uvicorn multi_agent.backend:app --host 127.0.0.1 --port 8050 --ws-ping-interval 25 --ws-ping-timeout 90
