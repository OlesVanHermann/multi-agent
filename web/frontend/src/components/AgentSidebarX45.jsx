...
function AgentSidebarX45({ agents, triangles, selectedAgent, controlAgent, onAgentClick, onFileClick, agentNames = {}, chatElement, username = 'default' }) {
  ...
  const getTriangleColors = (workerId) => {
    ...
    const hasRed = statuses.some(s => s === 'context_compacted' || s === 'error' || s === 'blocked')
    const hasOrange = statuses.some(s => s === 'context_warning')
                ...
                <div className="fav-bar">
                  {favEntries.map(([aid, pos]) => {
                    ...
                    const { fillColor, borderColor, isPulsing } = isTriangle
                      ...
                      : { fillColor: getStatusColor(agentMap[aid]?.status), borderColor: '', isPulsing: agentMap[aid]?.status === 'context_compacted' }
              ...
              <div key={ri} className="x45-grid">
                {row.map(a => {
                  ...
                  const { fillColor, borderColor, isPulsing } = isTriangle
                    ...
                    : { fillColor: getStatusColor(a.status), borderColor: '', isPulsing: a.status === 'context_compacted' }
