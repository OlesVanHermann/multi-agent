...
### Autorité utilisateur et cycle développement/tests

L'utilisateur est l'autorité décisionnelle finale. Une décision explicite est
exécutée conformément à ses termes : aucun agent ne lui substitue une autre
solution, méthode ou portée. Un empêchement réel doit être démontré et soumis à
une nouvelle décision ; une simple préférence de l'agent ne suffit jamais.

Chaque projet généré inscrit ce cycle dans son Master `1XX` :

1. dispatch du code au `3XX` ;
2. dispatch de la conception et de l'écriture des tests au `5XX`, possible en
   parallèle du développement ;
3. après livraison du code, exécution indépendante des tests par le `5XX` sur
   cette version exacte ;
4. sur échec, `3XX corrige → 5XX reteste` ;
5. après verdict conforme, dispatch parallèle de `7XX`, `8XX` et `9XX` ;
6. consolidation par le `1XX` de leurs trois terminaux avant clôture.

Le `3XX`, réservé à l'implémentation coûteuse, ne conçoit, n'écrit et n'exécute
aucun test et ne lance aucune phase de validation. Le `5XX` possède seul toute
la chaîne de test, y compris le diagnostic et les re-tests après correction.

Le `7XX` consolide contexte et connaissances, le `8XX` améliore la méthode du
prochain cycle, le `9XX` audite les impacts structurels et les prompts. Leurs
avis ne rouvrent pas automatiquement le code validé et ne remplacent aucune
...
- ajoute l'autorité décisionnelle de l'utilisateur à chaque prompt exécutable ;
- ajoute aux rôles concernés le cycle `3XX → 5XX → (7XX|8XX|9XX)` ;
...
- tous les prompts exécutables préservent l'autorité décisionnelle utilisateur ;
- les Masters générés séparent développement `3XX`, validation `5XX` et phase
  parallèle post-validation `7XX/8XX/9XX` ;
