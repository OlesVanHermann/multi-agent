...
Une tâche planifiée de 360 minutes peut demander au Contradictor d'optimiser le
prompt périodique de son Master. Il collecte l'état réel, confronte la phase
courante aux décisions utilisateur et ne remplace que le texte de l'unique
`crontab/NNN-1XX_*.prompt[.suspended]` :

```bash
printf '%s\n' "$NOUVEAU_PROMPT" | ./scripts/contradictor.sh revise-prompt 301
```

Si le Master ne possède encore **aucun** prompt planifié, la même commande le
crée à son nom canonique `crontab/NNN-1XX_120.prompt.suspended` (période 2 h)
avec le texte reçu — toujours **suspendu** : l'activation reste exclusivement
opérateur. S'il existe plusieurs fichiers candidats, l'action est refusée.

Sur un fichier existant, l'ancien texte est sauvegardé sous
`removed/contradictor-crontab/`. Le nom, la période, le statut actif/suspendu
et les objectifs restent inchangés. Aucun autre fichier crontab ou prompt
d'agent n'est modifiable par cette action.

À sa création, le créateur écrit la tâche du 2XX à 6 h sous la forme
`NNN-2XX_360.prompt.suspended`. Lors du premier tir 6 h activé par l'opérateur,
le 2XX collecte l'état puis crée idempotemment la tâche du Master à 2 h sous
`NNN-1XX_120.prompt.suspended`, avec un texte dérivé de l'objectif utilisateur
et de l'état durable. Il ne l'active jamais. Aux tirs suivants, il utilise
`revise-prompt`. Les activations appartiennent exclusivement à l'opérateur.
...
python3 scripts/scaffold-observers.py 345 \
  ...
  --contradictor-login login4a
```

Le Contradictor x45/z21 utilise par défaut `gpt-5-6-sol`, `login4a` et
l'effort `M`
