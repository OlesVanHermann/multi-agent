...
```bash
cp /chemin/vers/cdp-bridge/chrome-bridge.py $BASE/framework/chrome-bridge.py
chmod +x $BASE/framework/chrome-bridge.py
```
...
```bash
# Naviguer vers Gmail
AGENT_ID=000 python3 $BASE/framework/chrome-bridge.py goto "https://mail.google.com/mail/u/0/#search/similarweb"

# Attendre le chargement
AGENT_ID=000 python3 $BASE/framework/chrome-bridge.py wait 3

# Cliquer sur le premier email
AGENT_ID=000 python3 $BASE/framework/chrome-bridge.py click-text "Vérification de la connexion"

# Attendre l'ouverture
AGENT_ID=000 python3 $BASE/framework/chrome-bridge.py wait 3

# Lire le contenu texte de la page
AGENT_ID=000 python3 $BASE/framework/chrome-bridge.py read-text /tmp/email.txt
...
```bash
CHROME="AGENT_ID=300 python3 $BASE/framework/chrome-bridge.py"
...
```bash
CHROME="AGENT_ID=360 python3 $BASE/framework/chrome-bridge.py"
