...
def test_scaffold_is_parameterized_for_new_install(tmp_path):
    ...
    periodic = tmp_path / "crontab" / "427-217_360.prompt.suspended"
    assert periodic.is_file()
    assert "revise-prompt" in periodic.read_text()
...
def test_x45_z21_role_model_matrix():
    # Matrice opérateur 2026-08-23 — nouveaux agents uniquement ; un agent
    # existant garde son affectation (aucune migration automatique).
    assert models_mod.ROLE_DEFAULTS == {
        "master": ("100", "sonnet-5", "login3a", "M"),
        "contradictor": ("200", "gpt-5-6-sol", "login4a", "M"),
        "developer": ("300", "fable-5", "login4a", "M"),
        "observer": ("500", "opus-5", "login1a", "M"),
        "curator": ("700", "gpt-5-6-sol", "login4a", "M"),
        "coach": ("800", "gpt-5-6-sol", "login1a", "M"),
        "architect": ("900", "gpt-5-6-sol", "login3a", "M"),
    }
...
def test_topology_assignments_use_existing_role_suffixes(tmp_path):
    ...
    assert by_suffix == {
        ...
        "261": ("gpt-5-6-sol", "login4a"),
        "361": ("fable-5", "login4a"),
        "561": ("opus-5", "login1a"),
        "761": ("gpt-5-6-sol", "login4a"),
        "861": ("gpt-5-6-sol", "login1a"),
        "961": ("gpt-5-6-sol", "login3a"),
    }
...
def test_mono_is_compound_main_plus_contradictor(tmp_path):
    ...
    (prompts / "login4a.login").write_text("login4a")
    ...
    main, contradictor = mono_pair_mod.scaffold(
        tmp_path, "345", "145", "245", "345-example", "gpt-5-6-sol", "login4a")
    ...
    assert (mono / "345-145.model").read_text() == "claude-opus-5"
    assert (mono / "345-145.login").read_text() == "login3a"
    ...
    assert (mono / "345-245.login").read_text() == "login4a"
    assert (tmp_path / "crontab" / "345-245_360.prompt.suspended").is_file()
