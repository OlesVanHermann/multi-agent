...
class TestCouverture:
    ...
    def test_optional_fable_5_1_is_distributed(self, manifest_paths):
        assert "fable-5-1.model" in bash_array(UPGRADE, "PROMPTS_FRAMEWORK_FILES")
        assert "prompts/fable-5-1.model" in manifest_paths

    def test_codex_0153_models_are_distributed(self, manifest_paths):
        models = {
            "gpt-6-astra.model", "gpt-5-5.model", "gpt-5-4.model",
            "gpt-5-4-mini.model", "gpt-5-3-codex-spark.model",
        }
        distributed = set(bash_array(UPGRADE, "PROMPTS_FRAMEWORK_FILES"))
        assert models <= distributed
        assert {f"prompts/{model}" for model in models} <= set(manifest_paths)
