...
def test_done_sh_closes_obligation_after_atomic_terminal_publish():
    source = (ROOT / "scripts" / "done.sh").read_text()
    publish = source.index('PUBLISH_OUTPUT=$(python3 "$BUS_PROTOCOL" terminal')
    result_check = source.index('if [ "$PUBLISH_RC" -ne 0 ]', publish)
    cleanup = source.index("close_obligation", result_check)
    assert publish < result_check < cleanup
    assert "$REDIS_CLI XADD \"completion\"" not in source
    assert 'XADD "$(agent_inbox_key "$TO_AGENT")"' not in source
