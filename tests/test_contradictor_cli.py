...
def test_revise_prompt_only_rewrites_master_crontab_and_preserves_status(tmp_path, monkeypatch):
    make_triangle(tmp_path)
    crontab = tmp_path / "crontab"
    crontab.mkdir()
    master = crontab / "345-145_120.prompt.suspended"
    master.write_text("ancien\n")
    other = crontab / "345-745_60.prompt"
    other.write_text("intact\n")
    monkeypatch.setattr(MODULE, "BASE", tmp_path)
    monkeypatch.setattr(MODULE.sys, "stdin", io.StringIO("nouveau ciblé\n"))

    MODULE.revise_prompt("345")

    assert master.read_text() == "nouveau ciblé\n"
    assert other.read_text() == "intact\n"
    assert not (crontab / "345-145_120.prompt").exists()
    backups = list((tmp_path / "removed" / "contradictor-crontab").glob(
        "*/345-145_120.prompt.suspended"))
    assert len(backups) == 1
    assert backups[0].read_text() == "ancien\n"


def test_revise_prompt_is_noop_when_text_is_unchanged(tmp_path, monkeypatch, capsys):
    make_triangle(tmp_path)
    crontab = tmp_path / "crontab"
    crontab.mkdir()
    (crontab / "345-145_120.prompt").write_text("optimal\n")
    monkeypatch.setattr(MODULE, "BASE", tmp_path)
    monkeypatch.setattr(MODULE.sys, "stdin", io.StringIO("optimal\n"))
...
def test_revise_prompt_creates_suspended_2h_task_when_master_has_none(tmp_path, monkeypatch, capsys):
    make_triangle(tmp_path)
    crontab = tmp_path / "crontab"
    crontab.mkdir()
    other = crontab / "345-745_60.prompt"
    other.write_text("intact\n")
    monkeypatch.setattr(MODULE, "BASE", tmp_path)
    monkeypatch.setattr(MODULE.sys, "stdin", io.StringIO("premier prompt\n"))

    MODULE.revise_prompt("345")

    created = crontab / "345-145_120.prompt.suspended"
    assert created.read_text() == "premier prompt\n"
    assert not (crontab / "345-145_120.prompt").exists()
    assert other.read_text() == "intact\n"
    payload = json.loads(capsys.readouterr().out.strip())
    assert payload == {
        "status": "created", "target": "345-145",
        "file": "crontab/345-145_120.prompt.suspended",
        "period": 120, "suspended": True,
    }
    assert not (tmp_path / "removed").exists()


def test_revise_prompt_refuses_when_master_has_several_candidates(tmp_path, monkeypatch):
    make_triangle(tmp_path)
    crontab = tmp_path / "crontab"
    crontab.mkdir()
    (crontab / "345-145_120.prompt").write_text("actif\n")
    (crontab / "345-145_60.prompt.suspended").write_text("suspendu\n")
    monkeypatch.setattr(MODULE, "BASE", tmp_path)
    monkeypatch.setattr(MODULE.sys, "stdin", io.StringIO("nouveau\n"))

    try:
        MODULE.revise_prompt("345")
        raise AssertionError("SystemExit attendu")
    except SystemExit as exc:
        assert "ambigu" in str(exc)
