...
def test_scheduled_tasks_offer_six_hours():
    config = (ROOT / "web/backend/multi_agent/config.py").read_text()
    frontend = (ROOT / "web/frontend/src/components/CrontabSplit.jsx").read_text()
    assert "VALID_CRONTAB_PERIODS = {10, 30, 60, 120, 360}" in config
    assert "[10, 30, 60, 120, 360]" in frontend
    assert "v === 360 ? '6h'" in frontend


def test_scheduler_fires_every_period_the_api_accepts():
    """Un fichier accepté par l'API mais refusé par le scheduler serait
    ignoré en silence (« SKIP ... invalid period ») — cas réel v3.2.22 :
    les tâches NNN-2XX_360 (revise-prompt-1XX) n'auraient jamais tiré."""
    spec = importlib.util.spec_from_file_location(
        "crontab_scheduler", ROOT / "scripts" / "crontab-scheduler.py")
    sched = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(sched)
    assert sched.VALID_PERIODS == {10, 30, 60, 120, 360}
    for period in sched.VALID_PERIODS:
        assert any(sched.is_aligned(m, h, period)
                   for h in range(24) for m in range(60)), period
    assert [h for h in range(24) if sched.is_aligned(0, h, 360)] == [0, 6, 12, 18]
    assert not sched.is_aligned(30, 6, 360)
    assert sched.fire_key(6, 0, 360) != sched.fire_key(12, 0, 360)
