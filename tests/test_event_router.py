...
def test_same_message_can_resume_after_decision_reservation_crash():
    """La réservation NX précède la queue : après crash, le même PEL doit
    pouvoir reprendre au lieu d'être acquitté comme un faux doublon."""
    agent = bridge("334-134")
    message = envelope(
        "DECISION_REQUIRED",
    ...
    key = "inbound_decision:334-134:decision-1"
    reservations = {key: "42-0"}
    ...
    agent._handle_inbox_message("42-0", message)

    assert agent.prompt_queue.qsize() == 1
    assert agent._ack_inbox.call_count == 0


def test_report_wake_and_done_terminal_open_one_model_decision():
    agent = bridge("334-134")
    reservations = {}
    ...
    terminal = envelope(
        "INFO_REQUIRED", **common, event_id="terminal-1")

    agent._handle_inbox_message("1-0", wake)
    agent._handle_inbox_message("2-0", terminal)

    assert agent.prompt_queue.qsize() == 1
    task = agent.prompt_queue.get_nowait()
    assert task["decision_id"] == "decision-1"
    assert task["source_report_id"] == "report-1"
    agent._ack_inbox.assert_called_once_with("2-0")


def test_nonblocking_message_never_consumes_done_decision_slot():
    """decision_id est transporté pour l'audit sur toutes les enveloppes,
    mais seuls BLOCKED/INFO_REQUIRED partagent un réveil dédupliqué."""
    agent = bridge("334-134")
    reservations = {}
    ...
    common = {
        ...
        "correlation_id": "corr-1",
        "source_turn_id": "source-turn-1",
        "decision_id": "decision-shared-for-audit",
    }

    agent._handle_inbox_message(
        "1-0", envelope("MESSAGE", **common, event_id="message-1"))
    agent._handle_inbox_message(
        "2-0", envelope("DONE", **common, event_id="terminal-1"))

    assert event_router.decision_identity(
        envelope("MESSAGE", **common)) == ""
    assert event_router.decision_identity(
        envelope("DONE", **common)) == ""
