...
def make_base(tmp_path):
    ...
    for name in ("fable-5.model", "gpt-5-6-sol.model", "opus-5.model",
                 "login1a.login", "login3a.login", "login4a.login", "codex3a.login"):
...
def test_adds_contradictor_to_existing_x45_idempotently(tmp_path):
    ...
    assert (directory / "427-227.model").readlink() == Path("../gpt-5-6-sol.model")
    assert (directory / "427-227.login").readlink() == Path("../login4a.login")
    assert (directory / "427-227.effort").read_text() == "M\n"
    assert MIGRATOR.plan(base) == ([], [])
