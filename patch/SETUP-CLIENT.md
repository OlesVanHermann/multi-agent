...
- Git installé
- Clé SSH configurée pour le hub : `ssh ubuntu@mx9.di2amp.com` doit fonctionner
...
# Vérifier
git remote -v
# hub    ubuntu@mx9.di2amp.com:/home/ubuntu/multi-agent.git (fetch)
# hub    ubuntu@mx9.di2amp.com:/home/ubuntu/multi-agent.git (push)
# origin https://github.com/OlesVanHermann/multi-agent.git (fetch)
# origin https://github.com/OlesVanHermann/multi-agent.git (push)
```
...
**Important** : les agents Claude commitent avec le co-auteur `Co-Authored-By: Claude Sonnet 4.6` mais l'auteur doit rester `OlesVanHermann`.
...
cd ~/multi-agent
git ls-files scripts web docs patch setup tests templates examples framework | xargs md5 > file.md5
...
~/multi-agent/          ← WORKING COPY (code + projet)
  scripts/secrets.cfg   ← secrets (gitignored)
