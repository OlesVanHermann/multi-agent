...
case "$CURRENT" in
    main) RELEASE_LINE="3.2"; RELEASE_REMOTE_BRANCH="main" ;;
...
FRAMEWORK_PATHS=(scripts web docs patch setup tests templates examples framework .github bench
                 ...
                 prompts/gpt-6-astra.model prompts/gpt-5-5.model prompts/gpt-5-4.model prompts/gpt-5-4-mini.model prompts/gpt-5-3-codex-spark.model
                 prompts/gpt-5-6-luna.model prompts/gpt-5-6-sol.model prompts/gpt-5-6-terra.model
                 prompts/opus-5.model prompts/fable-5.model prompts/fable-5-1.model prompts/sonnet-5.model
                 'prompts/codex*.login'
                 requirements.txt CLAUDE.md AGENTS.md README.md LICENSE .gitignore)
# Garde C3 : sha256sum lit le CONTENU DE L'ARBRE DE TRAVAIL. Un fichier
# framework tracké mais modifié non commité empoisonne le manifest — vécu sur
# la release v3.2.22 (login/claude1a+4a/settings.json modifiés sur le hub,
# jamais commités : upgrade.sh a refusé la release sur toutes les machines).
# Refuser de générer le manifest tant que ces chemins ne sont pas propres.
DIRTY_FRAMEWORK=$(git status --porcelain -- "${FRAMEWORK_PATHS[@]}" || true)
if [ -n "$DIRTY_FRAMEWORK" ]; then
    echo -e "${RED}ERROR: fichiers framework modifiés non commités — committer ou remiser avant release :${NC}" >&2
    echo "$DIRTY_FRAMEWORK" | head -20 >&2
    exit 1
fi
