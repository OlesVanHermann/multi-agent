#!/usr/bin/env python3
"""
Crawl v2 - Version sans filtre de langue obligatoire

Difference avec crawl.py:
- INCLUDE_PATTERNS vide par defaut (accepte toutes les URLs du domaine)
- Utiliser config.json pour ajouter des filtres si necessaire

Utiliser crawl2.py quand:
- Le site n'a pas de prefixes de langue (/en/, /fr/)
- crawl.py ne decouvre aucune URL malgre des liens presents

Usage: python3 crawl2.py <domaine>
"""
...
# DIFFERENCE MAJEURE: Pas de filtre include par defaut
# Accepte toutes les URLs du meme domaine
INCLUDE_PATTERNS = []  # Vide = accepter tout

EXCLUDE_PATTERNS = [
    r'\?',     # Exclure les URLs avec parametres
    r'#',      # Exclure les ancres
    r'/docs/', # Exclure la doc (trop volumineux)
    r'/api/',  # Exclure les endpoints API
    r'/cdn-cgi/', # Exclure Cloudflare
...
def should_include_url(url: str, config: dict) -> bool:
    ...
    # DIFFERENCE: Si include est vide, on accepte tout (pas de filtre)
    if include:
...
async def main():
    ...
    if len(args) < 1:
        ...
        print("Exemple: python3 crawl2.py hetzner.com")
        print("         python3 crawl2.py mistral.ai --subdomains")
        print("         python3 crawl2.py zoom.us --agent-id=320")
        print("\nOptions:")
    ...
    print(f"Mode: crawl2 (sans filtre langue)")
